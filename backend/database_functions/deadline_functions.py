...
import datetime
from database_functions.checkpoint_functions import set_checkpoints_function, deleting_existing_checkpoints_for_course
...
def set_deadline_function(user_id, date, course_id):
    ...
    if id == None:
        ...
        set_checkpoints_function(user_id, course_id, datetime.datetime.now().date(), deadline_as_date, 3)
        db.session.commit()
